...
from pyrogram.errors import ChatAdminRequired, FloodWait, MediaEmpty, FileReferenceExpired, FileIdInvalid
...
async def stream_link(client, message):
    """Generate a 24-hour stream/download link for a replied-to media file."""
    if not STREAM_MODE:
        return await message.reply_text("Streaming is currently disabled.")

    replied = message.reply_to_message
    if not replied or not replied.media:
        return await message.reply_text(
            "Reply to a video, document, or audio file with /stream."
        )
    ...
    if media_type not in [enums.MessageMediaType.VIDEO, enums.MessageMediaType.AUDIO, enums.MessageMediaType.DOCUMENT]:
        return await message.reply_text(
            "Please reply to a video, document, or audio file with /stream."
        )

    status = await message.reply_text("Generating stream and download links...")
    try:
        file = getattr(replied, media_type.value)
        # Copy the media into the log/store channel so the existing streaming
        # route can serve it reliably, then generate signed 24-hour links.
        log_msg = await client.send_cached_media(chat_id=LOG_CHANNEL, file_id=file.file_id)
        stream, download = make_stream_links(
            log_msg.id,
            quote_plus(get_name(log_msg)),
            get_hash(log_msg)
        )
        ...
        await status.edit_text(
            "<b>Here are your stream and download links.</b>\n\n"
            "⏰ <b>Link validity:</b> 24 hours",
            reply_markup=InlineKeyboardMarkup(buttons)
        )
    except Exception as e:
        logger.exception("/stream failed")
        await status.edit_text(f"Failed to generate stream link.\n<code>{e}</code>")
...
async def start(client, message):
    ...
    if data.startswith("sendfiles"):
        ...
        for file in files:
            ...
            except (MediaEmpty, FileReferenceExpired, FileIdInvalid) as e:
                logger.warning(
    ...
    try:
        ...
        return await message.reply(
            ...
            "Please try searching again — it's been removed from "
            "results so this won't happen again for this file."
        )
    except Exception as e:
        logger.exception("Failed to send file %s", file_id)
        return await message.reply(
            "<b>❌ Something went wrong sending that file. "
            "Please try again in a moment.</b>"
        )
    btn = [[InlineKeyboardButton("✅ ɢᴇᴛ ғɪʟᴇ ᴀɢᴀɪɴ ✅", callback_data=f'del#{file_id}')]]
    k = await msg.reply(text=f"<blockquote><b><u>❗️❗️❗️IMPORTANT❗️️❗️❗️</u></b>\n\nᴛʜɪs ᴍᴇssᴀɢᴇ ᴡɪʟʟ ʙᴇ ᴅᴇʟᴇᴛᴇᴅ ɪɴ <b><u>10 mins</u> 🫥 <i></b>(ᴅᴜᴇ ᴛᴏ ᴄᴏᴘʏʀɪɢʜᴛ ɪssᴜᴇs)</i>.\n\n<b><i>ᴘʟᴇᴀsᴇ ғᴏʀᴡᴀʀᴅ ᴛʜɪs ᴍᴇssᴀɢᴇ ᴛᴏ ʏᴏᴜʀ sᴀᴠᴇᴅ ᴍᴇssᴀɢᴇs ᴏʀ ᴀɴʏ ᴘʀɪᴠᴀᴛᴇ ᴄʜᴀᴛ.</i></b></blockquote>")
    await asyncio.sleep(600)
...
async def stop_button(bot, message):
    await bot.send_message(
        text=(
            "**🔄 Restart request accepted. The bot is restarting now. "
            "A restart notice will be posted to the log channel once it is ready.**"
        ),
        chat_id=message.chat.id,
    )
